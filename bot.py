...
def get_pato_orange_markup():
    m=types.InlineKeyboardMarkup(row_width=1)
    for d,c in [("1 DAY - ₹60","pato_orange_plan_60"),("3 DAY - ₹130","pato_orange_plan_130"),("7 DAY - ₹240","pato_orange_plan_240"),("15 DAY - ₹380","pato_orange_plan_380"),("30 DAY - ₹550","pato_orange_plan_550")]:
        m.add(types.InlineKeyboardButton(d,callback_data=c))
    m.add(types.InlineKeyboardButton("↩️ Back",callback_data="main_non_root"))
    return m
...
def handle_menu_clicks(call):
    ...
    if call.data == "main_root_app":
        try:
            ...
            pass
        send_payment_details(call.message.chat.id, "Root App", 50)
    ...
    # --- AINCARD KEY APP ---
        ...
        try:
            ...
            pass
        send_payment_details(call.message.chat.id, "Aincard Key App", 50)
    ...
    elif call.data=="non_root_pato_orange":
        bot.edit_message_text(chat_id=call.message.chat.id,message_id=call.message.message_id,text="🔹 **PATO ORANGE**\n\nSelect a plan:",reply_markup=get_pato_orange_markup(),parse_mode="Markdown")
    elif call.data.startswith("pato_orange_plan_"):
        send_payment_details(call.message.chat.id,"PATO ORANGE",int(call.data.split("_")[-1]))
...
# --- BRAND SELECT HUA YA PAGE BADLA (Format: brand_<brandId>_<page>) ---
        ...
        nav=[]
        if page>0:
            nav.append(types.InlineKeyboardButton("◀️ Previous", callback_data=f"brand_{brand_id}_{page-1}"))
        if end_idx < len(models_list):
            nav.append(types.InlineKeyboardButton("Next ▶️", callback_data=f"brand_{brand_id}_{page+1}"))
        if nav:
            markup.row(*nav)
        markup.add(types.InlineKeyboardButton("↩️ Brands", callback_data="back_to_brands"))
